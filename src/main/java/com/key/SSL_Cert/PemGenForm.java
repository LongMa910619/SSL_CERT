...
public class PemGenForm extends FormLayout {
    final TextField pemName_tf = new TextField("Pem file name");
	...
	final ComboBox keyStoreFile_cb = new ComboBox("Keystore file");
    ...
    private void configureComponents()
    {
		...
		keyStoreFile_cb.addItems(getKeyFiles(str));
		keyStoreFile_cb.setImmediate(true);
    	
    	pemName_tf.setInputPrompt("ex. mycsr.pem");
    	pemName_tf.setImmediate(true);
    ...
    {
    	keyStoreFile_cb.addValidator(new Validator() {
			@Override
    	...
    	pemName_tf.addValidator(new Validator() {
			@Override
			public void validate(Object value) throws InvalidValueException {
				// TODO Auto-generated method stub
				String code = (String)value;
				if (code.length() == 0)
				{
					throw new InvalidValueException("The PEM filename" + ERROR_EMPTY);
				}
			}
		});
    ...
    {
    	addComponents(keyStoreFile_cb, pemName_tf, storePwd_pf, generate_btn);
    }
    ...
    {
    	try {
    		keyStoreFile_cb.validate();
    		pemName_tf.validate();
    		String execStr = String.format("keytool -certreq -v -alias codesigncert -file %s -keystore %s -storepass %s", pemName_tf.getValue(), keyStoreFile_cb.getValue(), storePwd_pf.getValue());
